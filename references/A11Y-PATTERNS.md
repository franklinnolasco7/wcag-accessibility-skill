...
Practical, copy-paste-ready patterns for common accessibility requirements.
Each pattern is self-contained and linked from the main [SKILL.md](../SKILL.md).

---

## Modal Focus Trap

Trap keyboard focus inside a modal dialog so Tab/Shift+Tab cycle through its focusable
elements and Escape closes it.
...
The native `<dialog>` element handles focus trapping automatically—prefer it when browser
support allows.
...
Every input needs an associated label—either explicit (`for`/`id`) or implicit
(wrapping `<label>`).
...
Also applies to sliders, map panning, color pickers, and similar drag-based
widgets—always provide an equivalent click/tap or keyboard path.

---

## ARIA Tabs

Tabs require `role="tablist"`, `role="tab"`, and `role="tabpanel"` with proper
`aria-selected`, `aria-controls`, and keyboard support.
...
Arrow keys should move focus between tabs; the active tab receives `tabindex="0"` while
...
Use `aria-live` to announce dynamic content changes to screen readers without
moving focus.
